...
from utils import train, test, train_adj, test_adj, lambda_train, lambda_test
...
def tune(log_path: str, conf_file: str, gpu: str) -> Callable:
    ...
    def objective(trial: Trial) -> float:
        ...
        cfg.MODEL.PARAM.k = trial.suggest_int("k", 1, 3)
        ...
        try:
            lambda_path = os.path.join(cfg.DATASET.PARAM.path,
                                    "lambda",
                                    func2path[cfg.GRAPH.ADJ_MATRIX],
                                    f"{cfg.DATASET.PARAM.name}")
        except KeyError:
...
if __name__ == "__main__":
    # args = ["--log_path", "/raid/zhouyz/log",
    #         "--task_type", "raw_phase_locking_value_ChebNet_JapaneseVowels",
    #         "--gpu", "0"]
