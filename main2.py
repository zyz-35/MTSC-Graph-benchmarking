...
def main(log_path: str, conf_file: str, save_path: str, gpu: int) -> Callable:
    ...
    Dataset = datasets["UEADataset2"]
    logger.info("=====> Preparing data...")
    end = time.time()
    dataset_param = dict(cfg.DATASET.PARAM)
    dataset_param["node"] = cfg.GRAPH.NODE
    dataset_param["adj"] = cfg.GRAPH.ADJ_MATRIX
    trainset = Dataset(**dataset_param, train=True)
    testset = Dataset(**dataset_param, train=False)
    ...
    for epoch in tqdm(range(cfg.EXPERIMENT.EPOCHS)):
        ...
        train(trainloader, model, loss_fn, optimizer, epoch, logger,
            device)
...
if __name__ == "__main__":
    args = ["--log_path", "~/debug",
            "--conf_file", "~/r1conf/raw-mutual_information-stgcn-2-FaceDetection.yml",
            "--save_path", "~/debug",
            "--gpu", "3"]
