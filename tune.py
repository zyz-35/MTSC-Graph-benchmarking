...
from utils import train_adj, test_adj
...
def tune(log_path: str, conf_file: str, gpu: str) -> Callable:
    ...
    def objective(trial: Trial) -> float:
        ...
        # 需要搜索的超参
        cfg.MODEL.PARAM.dropout = trial.suggest_float("dropout", 0.2, 0.5)
        cfg.MODEL.PARAM.n_layers = trial.suggest_int("n_layers", 1, 2)
        cfg.MODEL.PARAM.num_heads = 2**trial.suggest_int("num_heads", 0, 2)
        cfg.MODEL.PARAM.thred = \
            trial.suggest_float("thred", 0.2, 0.8, step=0.05)
        cfg.EXPERIMENT.OPTIMIZER.PARAM.lr = \
            trial.suggest_float("lr", 1e-5, 1e-2)
        cfg.EXPERIMENT.OPTIMIZER.PARAM.momentum = \
            trial.suggest_float("momentum", 0, 1.0)
        cfg.EXPERIMENT.OPTIMIZER.PARAM.weight_decay = \
            trial.suggest_float("weight_decay", 0, 1.0)
