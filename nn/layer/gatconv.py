...
class GATConv(nn.Module):
    ...
    def forward(self, adj: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        # 处理无边连接的结点，softmax为nan一般是分母为0，即孤立点，可以直接置0
        # att = att.nan_to_num()
        # masked_scores = all_scores + connectivity_mask.unsqueeze(1)
        # masked_scores -= masked_scores.max(dim=-1, keepdim=True)[0]
        # att = self.softmax(masked_scores)
...
if __name__ == "__main__":
    ...
    conv = GATConv(144, 32, 4, 0.5, 0.8, True)
    print(conv)
    pred = conv(adj, x)
    print(pred.shape)